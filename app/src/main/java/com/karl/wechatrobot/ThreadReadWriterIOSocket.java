...
import com.karl.wechatrobot.utils.FileHelper;
import com.karl.wechatrobot.utils.MyUtil;
...
public class ThreadReadWriterIOSocket implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			...
			ServiceUsbConnection.ioThreadFlag = true;
			while (ServiceUsbConnection.ioThreadFlag)
			{
				try
				{
					...
					/* 接收PC发来的数据 */
					Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "will read......");
					/* 读操作命令 */
					currCMD = readCMDFromSocket(in);
					Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "**currCMD ==== " + currCMD);
		...
		} finally
		{
			try
			{
				if (client != null)
				{
					Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "client.close()");
					client.close();
				}
			} catch (IOException e)
			{
				Log.e(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "read write error333333");
				e.printStackTrace();
			}
		}
	}
	...
	public static byte[] receiveFileFromSocket(InputStream in, OutputStream out, byte[] filelength, byte[] fileformat)
	{
		...
		try
		{
			...
			Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "read file OK:file size="
					+ filebytes.length);
			out.write("read file ok".getBytes("utf-8"));
			out.flush();
		} catch (Exception e)
		{
			Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "receiveFileFromSocket error");
			e.printStackTrace();
		}
		return filebytes;
	}
	...
	public String readCMDFromSocket(InputStream in)
	{
		...
		{
			Log.v(ServiceUsbConnection.TAG, Thread.currentThread().getName() + "---->" + "readFromSocket error");
			e.printStackTrace();
		}
